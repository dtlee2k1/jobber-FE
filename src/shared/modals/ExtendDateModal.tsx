...
import { IExtendedDelivery, IOrderDocument } from 'src/interfaces/order.interface'
import { useRequestDeliveryDateExtensionMutation } from 'src/services/order.service'

import Button from '../button/Button'
import Dropdown from '../dropdown/Dropdown'
import TextAreaInput from '../inputs/TextAreaInput'
import { dayMonthYear } from '../utils/timeago.utils'
import { showErrorToast, showSuccessToast } from '../utils/utils.service'
import ModalBg from './ModalBg'

interface IExtendedDateModalProps {
  order: IOrderDocument
  onClose: () => void
}

export default function ExtendDateModal({ order, onClose }: IExtendedDateModalProps) {
  ...
  const requestExtension = async () => {
    try {
      ...
      const response = await requestDeliveryDateExtension({ orderId: order.orderId, body: extended }).unwrap()
        ...
        <div className="relative bottom-auto left-auto right-auto top-auto max-h-[90vh] min-w-[350px] bg-white p-4">
          ...
          <div>
            <div className="mb-5">
              ...
              <span className="text-[13px]">{dayMonthYear(order.offer.oldDeliveryDate)}</span>
            </div>
            ...
            <div className="mb-5">
              <h4 className="mb-[5px] text-sm font-bold">Reason</h4>
              <TextAreaInput
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              className={`rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base ${
                !reason || !deliveryDate ? 'cursor-not-allowed bg-sky-200' : 'cursor-pointer bg-sky-500 hover:bg-sky-400 '
              }`}
              label="Send Request"
