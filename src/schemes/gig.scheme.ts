...
const gigInfoSchema: ObjectSchema<ICreateGig | any> = object({
  ...
  coverImage: string()
    .notOneOf(['https://placehold.co/330x220?text=Cover+Image'], { coverImage: 'Gig cover image is a required field' })
    .required({ coverImage: 'Gig cover image is a required field' }),
  ...
  price: number().min(3, { price: 'The lowest price must be $3' })
})
