...
import { IRatingCategories, IRatingCategoryItem, IReviewDocument } from 'src/interfaces/review.interface'
import { useGetReviewsByGigIdQuery } from 'src/services/review.service'
...
export default function GigViewReviews({ showRatings, hasFetchedReviews, reviews }: IGigViewReviewsProps) {
          ...
          reviews.map((item: IReviewDocument) => (
            <div key={uuidv4()}>
              <div className="flex flex-col gap-y-3 md:flex-row md:gap-x-4">
                <img
                  className="mt-4 flex h-12 w-12 self-center rounded-full object-cover md:self-auto"
                ...
                <div className="flex flex-col self-center">
                  ...
                  <div className="flex w-full justify-center gap-x-1 self-center md:justify-start">
                    <div className="pointer-events-none mt-1 w-20 gap-x-2">
                    ...
                    <div className="ml-2 mt-[1px] flex gap-1 text-sm">
                      ...
                      <span>{chatMessageTransform(`${item.createdAt}`)}</span>
                    </div>
                  </div>
                  <p className="mt-2 text-center text-sm md:text-left md:text-base">{item.review}</p>
                </div>
              </div>
            </div>
          ))}
