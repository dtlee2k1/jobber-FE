...
export default function GigRelatedTags() {
        ...
        {gig?.tags.map((tag: string) => (
          <Link
            ...
            to={{
              ...
              search: createSearchParams({ query: replaceSpacesWithDash(tag) }).toString()
            }}
          >
            <span className="text-medium left-0 top-0 rounded-md bg-[#edeef3] p-2 font-bold text-[#55545b] hover:text-sky-400 hover:underline">
              {tag}
            </span>
          </Link>
        ))}
