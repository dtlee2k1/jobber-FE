    ...
    try {
      ...
      const pathname =
        location.pathname === '/inbox'
          ? `${location.pathname}/${lowerCase(chatUsername)}/${message.conversationId}`
          : location.pathname.replace(`/${username}/${conversationId}`, `/${lowerCase(chatUsername)}/${message.conversationId}`)
      navigate(pathname)

      socket.emit('getLoggedInUsers')
      if (message.receiverUsername === authUser?.username && lowerCase(`${message.senderUsername}`) === username) {
          ...
          >
            ...
            <div className="w-full text-sm dark:text-white">
              <div className="flex justify-between pb-1 font-bold text-[#777d74]">
                <span>{data.receiverUsername !== authUser?.username ? data.receiverUsername : data.senderUsername}</span>
