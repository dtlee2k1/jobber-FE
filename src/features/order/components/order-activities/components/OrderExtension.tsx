...
import { IExtendedDelivery } from 'src/interfaces/order.interface'
import { useUpdateDeliveryDateMutation } from 'src/services/order.service'
import Button from 'src/shared/button/Button'
import ApprovalModal from 'src/shared/modals/ApprovalModal'
import { compareDates, dayMonthYear, dayWithTime } from 'src/shared/utils/timeago.utils'
  ...
  return (
    <>
      ...
      {order?.requestExtension &&
        ...
        compareDates(order.offer.oldDeliveryDate, order.offer.newDeliveryDate) === 0 && (
                    ...
                    <div className="mt-2">
                      <div className="px-4 py-1 text-left text-sm text-gray-500">
                        <div className="flex flex-col md:grid md:grid-cols-3">
                          ...
                          <p className="col-span-2 text-sm">{dayMonthYear(order?.requestExtension.originalDate)}</p>
                        </div>
                        <div className="mt-5 flex flex-col md:grid md:grid-cols-3">
                          ...
                          <p className="col-span-2 text-sm">{dayMonthYear(order?.requestExtension.newDate)}</p>
                        </div>
                        <div className="mt-5 flex flex-col md:grid md:grid-cols-3">
            ...
            <div className="flex gap-4">
              ...
              <div className="border-grey w-full border-b pb-6">
                <div className="flex items-center justify-between font-medium text-gray-500">
                  ...
                  <p className="text-sm font-normal italic">{dayWithTime(`${order?.events.deliveryDateUpdate}`)}</p>
                </div>
                <div className="border-grey mt-4 flex w-full flex-col rounded border">
                  <div className="mt-2">
                    <div className="px-4 py-1 text-left text-sm text-gray-500">
                      <div className="flex flex-col md:grid md:grid-cols-3">
                        <span className="col-span-1 text-sm font-bold">Original delivery date</span>
                        <p className="col-span-2 text-sm">{dayMonthYear(`${order?.offer.oldDeliveryDate}`)}</p>
                      </div>
                      <div className="mt-5 flex flex-col md:grid md:grid-cols-3">
                        <span className="col-span-1 text-sm font-bold">New delivery date</span>
                        <p className="col-span-2 text-sm">{dayMonthYear(`${order?.offer.newDeliveryDate}`)}</p>
                      </div>
                      <div className="mt-5 flex flex-col md:grid md:grid-cols-3">
                        <span className="col-span-1 text-sm font-bold">Reason</span>
