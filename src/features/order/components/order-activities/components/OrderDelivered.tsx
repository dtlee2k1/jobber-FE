...
import { IDeliveredWork, IOrderDeliveredModal, IOrderMessage } from 'src/interfaces/order.interface'
import { useApproveOrderMutation } from 'src/services/order.service'
import Button from 'src/shared/button/Button'
import ApprovalModal from 'src/shared/modals/ApprovalModal'
import { bytesToSize } from 'src/shared/utils/image-utils.service'
import { dayWithTime } from 'src/shared/utils/timeago.utils'
import { downloadFile, getFileBlob, showErrorToast, showSuccessToast } from 'src/shared/utils/utils.service'
...
const OrderDelivered = forwardRef<HTMLDivElement, IOrderDeliveredProps>((_, ref) => {
  ...
  const onDeliveryApprovalHandler = async () => {
    try {
  ...
  useEffect(() => {
    setOrderDeliveredModal({ ...orderDeliveredModal, delivery: true })
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [viewDeliveryBtnClicked])
              ...
              <div className="border-grey w-full border-b pb-5" ref={ref}>
                <div className="mt-2 flex items-center justify-between font-medium text-gray-500">
                  <div className="flex gap-2">
                    ...
                    <p className="flex self-center text-sm font-normal italic">{dayWithTime(`${order?.events.orderDelivered}`)}</p>
                  </div>
                  <div
            ...
            <div className="flex gap-4">
              ...
              <div className="border-grey w-full border-b pb-6">
                <div className="mt-2 flex items-center gap-2 font-medium text-gray-500">
                  ...
                  {order.approved && <p className="text-sm font-normal italic">{dayWithTime(`${order?.approvedAt}`)}</p>}
                </div>
                {!order.approved && authUser?.username === order.buyerUsername && (
                  <div className="my-3 flex flex-col">
                    <div className="relative overflow-x-auto">
