...
export interface IOrderDetailsTableProps {
  order: IOrderDocument
  authUser: IAuthUser
}

export default function OrderDetailsTable({ authUser, order }: IOrderDetailsTableProps) {
  const [showOrderDetailsPanel, setShowOrderDetailsPanel] = useState<boolean>(false)
  const orderInvoice = useRef<IOrderInvoice>()
        ...
        <div className="flex gap-4">
          ...
          <div className="w-full">
            <div className="mt-2 flex items-center justify-between font-medium text-gray-500">
              ...
              <div className="cursor-pointer" onClick={() => setShowOrderDetailsPanel(!showOrderDetailsPanel)}>
                {!showOrderDetailsPanel ? <FaChevronDown size={15} /> : <FaChevronUp size={15} />}
              </div>
            </div>
            {order && showOrderDetailsPanel && (
              <div className="my-3 flex flex-col">
                <div className="flex justify-between">
                  <div className="flex gap-2 text-sm md:text-base">
                    <div className="flex gap-2">
                      Buyer:
                      <span className="font-bold text-blue-400">{order.buyerUsername}</span>
                    </div>
                    <div className="flex gap-1">|</div>
                    <div className="flex gap-2">
                      Date ordered <p className="font-bold italic">{dayMonthYear(`${order?.dateOrdered}`)}</p>
                    </div>
                  </div>
                  {order.buyerUsername === authUser.username && orderInvoice.current && (
                    <PDFDownloadLink
                      document={
                    ...
                    <table className="border-grey w-full border text-left text-sm text-gray-500">
                      ...
                      <tfoot className="bg-[#f3f3f3]">
                        <tr>
                          ...
                          <td className="px-4 py-3"></td>
                          <td className="px-4 py-3 font-bold">${order.serviceFee}</td>
                        </tr>
                        <tr>
                          <th scope="row" className="px-4 py-3 text-base">
                            Total
                          ...
                          <td className="px-4 py-3"></td>
                          <td className="px-4 py-3"></td>
                          <td className="px-4 py-3 font-bold">${order.price + parseFloat(`${order.serviceFee}`)}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
